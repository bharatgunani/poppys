        ...
        if (in_array($status, array_merge($invoicedStatuses, $receivedStatuses, $canceledStatuses))) {
            ...
            foreach ($itemsCollection as $product) {
                if ($itemId = $product->getBuyRequest()->getItemId()) {
                    $this->loadWithOptions($itemId);
                    if ($this->getId() && ($this->getProductId() === $product->getProductId())) {

                        // Increase item qty
                        if (in_array($status, $invoicedStatuses)) {
                            // skip if we already changed QTY for this order
                            if ($this->getOptionByCode('order_id_invoiced')
                                && $this->getOptionByCode('order_id_invoiced')->getValue() == $order->getId()) {
                                continue;
                            }

                            $this->setQtyOrdered($this->getQtyOrdered() + $product->getQtyOrdered());
                            $this->addOption([
                            ...
                            $newOrderedItems[$this->getId()] = (int) $product->getQtyOrdered();
                        }

                        if (in_array($status, $receivedStatuses)) {
                            // skip if we already changed QTY for this order
                            if ($this->getOptionByCode('order_id_received')
                                && $this->getOptionByCode('order_id_received')->getValue() == $order->getId()) {
                                continue;
                            }

                            $this->setQtyReceived($this->getQtyReceived() + $product->getQtyOrdered());
                            $this->addOption([
                                'product_id' => $this->getProductId(),
                        ...
                        if (in_array($status, $canceledStatuses)) {
                            ...
                            if ($this->getQtyReceived() > 0 && in_array($origStatus, $receivedStatuses)) {
                                $this->setQtyReceived($this->getQtyReceived() - $product->getQtyOrdered());
                            }
                        }

                        if ($this->dataHasChangedFor('qty_ordered') || $this->dataHasChangedFor('qty_received')) {
                            $this->save();
                        }
                    }
                }
