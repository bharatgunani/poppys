...
\Magento\Shipping\Model\Carrier\CarrierInterface {
            ...
            if (!empty($rate) && $rate['price'] >= 0) {
			   ...
			   $address = $quote->getShippingAddress();
                ...
                if ($coupon) {
                    ...
                    $couponObj = $objectManager->get('\Magento\SalesRule\Model\Coupon');
                    $codes = $couponObj->loadByCode($coupon);
                    $ruleid = $codes->getData('rule_id');
                    $ruledetail = $objectManager->get('\Magento\SalesRule\Model\Rule');
                    $allobj = $ruledetail->load($ruleid);
                    //echo '<pre>';print_r($allobj->getData('freeshipment'));exit; 
                    $freeshipment = $allobj->getData('freeshipment');

                    if ($freeshipment == '1') {
                        ############### CODE FOR SET TAX RATE 0 WHILE USER APPLY COPOUN CODE ###
                        foreach ($quote->getAllItems() as $item) {
                            $product = $item->getProduct();
                            $product->setTaxClassId(0); /*Setting Price Tax Price 0*/
                            // here you can assign tax to the product depending on your conditions
                        }
                        #######################################################################
                        
                        if ($rate['shipping_method'] == '8 To 15 Working Days') {
                            $method->setPrice(0);
                            $method->setCost(0);
                        } else if ($rate['shipping_method'] == '2 To 4 Working Days') {
                            if ($countrycode == 'PA') {
                                $method->setPrice(2.95);
                                $method->setCost(2.95);
                            } else {
                                $method->setPrice(7.95);
                                $method->setCost(7.95);
                            }
                        } else {
    ...
    public function getRate(\Magento\Quote\Model\Quote\Address\RateRequest $request, $zipRange) {
        $result = $this->_matrixrateFactory->create()->getRate($request, $zipRange);
